...
export function ChatInput({ onSendMessage, disabled }: ChatInputProps) {
  ...
  useEffect(() => {
    if (textareaRef.current) {
      ...
      textareaRef.current.style.height = `${Math.min(textareaRef.current.scrollHeight, 192)}px`; // 192px = 48rem max-h
    }
  }, [content]);
      ...
      <div className="relative flex items-end w-full rounded-3xl bg-zinc-900/80 border border-zinc-800 focus-within:border-zinc-700 focus-within:ring-1 focus-within:ring-zinc-700 shadow-xl transition-all">
        ...
        <div className="p-2 md:p-3">
          <button 
            ...
            className={`flex items-center justify-center w-10 h-10 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 focus:ring-white
